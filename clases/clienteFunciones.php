...
//funcion strlen para determinar la logitud del string
...
function validaPassword($password, $repassword){
    //hacemos una comparacion entre dos string 
    //de manera binaria para marcar que es diferente
...
{
    $sql =$con->prepare("INSERT INTO clientes(nombres, apellidos, email, telefono, documento, estatus, fecha_alta) VALUES(?,?,?,?,?, 1, now())");
    ...
    if($row =$sql->fetch(PDO::FETCH_ASSOC)){
        if(esActivo($usuario, $con)){
            if(password_verify($password, $row['password'])){
                ...
                $_SESSION['user_cliente']=$row['user_cliente'];
                if($proceso == 'pago'){
